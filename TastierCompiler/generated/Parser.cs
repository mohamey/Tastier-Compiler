...
enum TastierType : int {   //Definition for data types in tastier
    Undefined,
    Integer,
    Boolean
  };
...
/*--------------------------------------------------------------------------*/
  ...
  public void Write (string filename) {
	...
	void Factor(out TastierType type) {
		int n; Symbol sym = null; string name; bool array = false; 
		...
		if (la.kind == 1) {
			...
			if (la.kind == 5) {
				...
				if (la.kind == 6) {
					...
					Expr(out type);
					Expect(6);
					name = name + xDimension + "" + expression; //Append X and Y dimansion of array
					array = true;
			...
			expression = n;         //Save last number inputted to compiler
	...
	void VarDecl(bool external) {
		string name; TastierType type; Scope currentScope = openScopes.Peek(); Symbol sym; TastierType typeStore;
		...
		sym = new Symbol(name, (int)TastierKind.Var, (int)type, openScopes.Count-1, currentScope.Count(s => (s.Item2 == (int)TastierKind.Var || s.Item2 == (int)TastierKind.Cons)));
		...
		while (la.kind == 7) {
			...
			if (external) {
			 ...
			 currentScope.Push(new Symbol(name, (int)TastierKind.Var, (int)type, openScopes.Count-1, currentScope.Count(s => (s.Item2 == (int)TastierKind.Var || s.Item2 == (int)TastierKind.Cons))));
			}
	...
	void Stat() {
		TastierType type; string name; Symbol sym = null; bool external = false; bool isExternal = false; bool array = false; 
		switch (la.kind) {
		case 1: {
			Ident(out name);
			if (la.kind == 5) {
	...
	void ConstDecl() {
		...
		Expect(40);
		Ident(out name);
		sym = new Symbol(name, (int)TastierKind.Cons, (int)TastierType.Integer, 0, currentScope.Count(s => (s.Item2 == (int)TastierKind.Var || s.Item2 == (int)TastierKind.Cons)));
		currentScope.Push(sym);//push symbol to scope
		type = TastierType.Integer;
	...
	void Tastier() {
		...
		}else if (s.Item2 == (int)TastierKind.Cons) {
	...
	static readonly bool[,] set = {
		...
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,T,T,T, T,T,T,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x},
		{x,T,x,x, x,x,x,x, x,x,x,x, T,x,x,T, x,x,x,x, x,x,x,x, x,x,x,T, x,T,x,x, T,T,x,T, T,x,T,T, x,x,x,x, x},
		{x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,T, x,x,x,x, x,x,x,x, x,x,x,T, x,T,x,x, T,T,x,T, T,x,x,x, x,x,x,x, x},
		{x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,T, x,x,x,x, x,x,x,x, x,x,x,T, x,T,x,x, T,T,x,T, T,x,T,T, T,x,x,x, x},
		{x,x,x,x, x,x,x,x, x,x,x,x, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,T,T, T,T,x,x, x}
...
public class Errors {
	...
	public virtual void SynErr (int line, int col, int n) {
		...
		switch (n) {
			...
			case 41: s = "\"external\" expected"; break;
			case 42: s = "\"procedure\" expected"; break;
